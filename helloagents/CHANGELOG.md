...
- UI：新增顶部“Command Palette / 搜索”快捷入口（含 `Ctrl/⌘ K` 提示），并统一首页/分类页搜索框、分类入口卡片、信息流卡片（含 compact 变体）与 SignalBoard 的交互反馈（hover/focus/层次阴影），提升可发现性与读扫效率。
- UX：首页（中日双语）新增 KPI 快捷区（`2h/24h/热度/稳定源`）并接入现有 quick filters 数据管线（`data-quick-*`），提供一跳筛选；同时补齐 hash 意图监听（`#search/#prefs/#cmdk`）的 `hashchange` 响应，保证站内点击锚点也可即时触发对应动作。
- UX：分类页（中日双语）同步引入 KPI 快捷区（按分类计算 `2h/24h/热度/稳定源`），并与首页使用同一视觉与交互语义，减少页面切换时的认知跳变。
...
- UI：修复首页/分类页“网格/列表”切换对桌面端无效的问题：列表默认不再强制 `PostList variant="compact"`，改为使用卡片流（含封面），并让 `data-acg-view="list"` 的 CSS 切换逻辑真正生效。
- UI：优化桌面首页布局为“主列/侧栏”独立堆叠，避免右侧信号板过高时把后续内容整体下推，造成左侧出现大块空白与“内容像被遮挡”的观感。
- UI：SignalBoard 容器查询阈值下调，使桌面侧栏在较窄宽度下也可启用 2 列排布，降低纵向压迫感并提升信息密度。
