...
- 补充：启动期 `data-acg-boot="1"` 与低性能模式会禁用大面积 `filter: blur(...)`（如 `.blur-3xl`），进一步降低首屏 paint 成本。
...
### 需求: 启动失败兜底（Boot Fatal Banner）

场景：静态站的交互层脚本初始化链可能因为浏览器扩展/网络/存储权限等原因异常中断，导致页面“看起来加载了但无法交互”，用户也缺少明确下一步。

- 预期结果：当初始化链被判定为 fatal 时，页面仍能通过**纯静态**的提示条告知“初始化失败”，并提供“一键刷新”入口，避免白屏/无响应体验。
- 实现要点：`src/layouts/SiteLayout.astro` 中渲染 `.acg-boot-fatal`（默认隐藏）；`src/styles/global.css` 在 `:root[data-acg-boot="failed"]` 下强制展示；客户端在捕获到启动级错误时写入该 dataset（同时可与 telemetry 记录联动），实现“错误边界式”的可见兜底。
...
场景：桌面端高频操作（命令面板/站内搜索）入口不够显性，信息流卡片与侧栏卡片在 hover/focus 的反馈层次不够统一，影响“扫一眼就能判读”的效率。

- 预期结果：顶部提供 Command Palette 快捷入口（`Ctrl/⌘ K` 提示）与搜索捷径；搜索框、分类入口卡片、信息流卡片（含 compact 变体）和 SignalBoard 统一交互反馈（边框高亮、阴影层级、触控降级），在不增加业务复杂度的前提下提升可发现性和阅读节奏。
- 交互一致性：首页（中日双语）新增 KPI 快捷区（`2h/24h/热度/稳定源`）并复用 `data-quick-*` 管线；同时补齐 `hashchange` 对 `#search/#prefs/#cmdk` 的监听，使站内锚点点击与外部深链行为一致。
- 页面一致性：分类页（中日双语）同步提供 KPI 快捷区，指标按当前分类数据计算（时间透镜/热度/稳定源），确保首页与分类页的“筛选引导节奏”一致。
