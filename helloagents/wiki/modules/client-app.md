...
- 预期结果：支持在“本页搜索/全站搜索”之间切换；全站模式下由 Web Worker 负责预取/解压 `public/data/posts.json(.gz)`、构建索引与执行筛选（IndexedDB 缓存 posts），主线程仅负责输入/虚拟网格渲染；已读变更通过 `acg:read-changed` 触发全站结果刷新；提供 `?` 快捷键提示筛选语法。
