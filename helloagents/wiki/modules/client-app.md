...
场景：在不引入完整主题系统的前提下，提供轻量但可感知的视觉个性化能力，让用户在不同设备上保持一致的站点氛围。

- 预期结果：支持 `neon/sakura/ocean/amber` 四种 Accent；持久化到本机；根节点注入 `data-acg-accent` 驱动全局样式；切换不影响核心业务逻辑。
- 实现要点：localStorage key 使用版本化命名（`acg.accent.v1`）；切换动作广播 `acg:accent-changed`，便于后续观测或扩展。

### 需求: 视图预设（View Presets）与深链复现

场景：用户希望保存一套常用“筛选 + 布局 + 主题/强调色”，并在跨设备/分享时可复现。

- 预期结果：视图预设支持保存/应用/重命名/删除，并可复制可复现的视图链接（打开后自动应用并清理 URL）。
- 实现要点：预设以版本化 store 存储在 localStorage（`acg.viewPresets.v1`）；应用通过事件 `acg:apply-view-preset` 复用既有 UI 逻辑；URL 使用白名单参数（如 `only/hide/lens/sort/view/density/theme/accent/scope/q`）。
...
- 能力补强：命令面板覆盖 View/Density（布局/密度）、Accent（强调色）与 View Presets（视图预设）等高频操作，确保不离开键盘即可改变阅读节奏与视觉偏好。
- 深链补强：支持 `/#search` 聚焦搜索、`/#prefs` 打开偏好设置抽屉、`/#cmdk` 直接打开命令面板（与键盘入口一致）；同时支持 `?q=` 与“视图快照参数”深链复现（应用后清理 URL，避免重复覆盖）。
